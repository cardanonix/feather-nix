...
myKeys conf@XConfig {XMonad.modMask = modm} =
  ...
  keySet "Scratchpads"
    ...
    , key "Gimp"            (modm .|. controlMask,  xK_g    ) $ runScratchpadApp gimp
...
myLayout =
  avoidStruts
    ...
    . imgLayout
    . ossLayout   
    . spoLayout $ (tiled ||| Mirror tiled ||| column3 ||| full ||| video_tile)
   where
      default tiling algorithm partitions the screen into two panes
     tiled        = gapSpaced 3 $ Tall nmaster delta ratio
     video_tile   = gapSpaced 2 $ Mirror (Tall 1 (2/100) (3/5))
     ...
     comLayout = onWorkspace comWs (full ||| tiled ||| video_tile)
     devLayout = onWorkspace devWs (column3 ||| full ||| goldenSpiral)
     musLayout = onWorkspace musWs (fuller ||| full ||| video_tile)
     webLayout = onWorkspace webWs (fuller ||| tiled ||| goldenSpiral ||| video_tile ||| full)
     spoLayout = onWorkspace spoWs (tiled ||| full ||| goldenSpiral)
     misLayout = onWorkspace misWs (tiled ||| full ||| fuller ||| goldenSpiral ||| video_tile)
     imgLayout = onWorkspace imgWs (fuller ||| tiled ||| video_tile) 
...
scratchpads = scratchpadApp <$> [ audacious, btm, nautilus, scr, spotify, vlc, mpv, gimp, kodi ]
...
projects =
  ...
  , Project { projectName      = spoWs
            , projectDirectory = "~/cardano_local/cardano-node/"
            , projectStartHook = Just . spawn $ "nix run github:input-output-hk/cardano-node#mainnet/node"
            }
  ...
  , Project { projectName      = imgWs
            ...
            , projectStartHook = Just $ runScratchpadApp gimp
            }
...
{- #TODO: Make Specific Apps Immune to FadeInactive
https://superuser.com/questions/260041/override-xmonad-fade-inactive-for-a-window -}